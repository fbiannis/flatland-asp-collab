...
class NaiveTimeInstance:
    ...
    def get_agents_header(self) -> list[str]:
        header = inspect.cleandoc(
            f"""% Agents are defined in two literals
                %   agent_start(I,X,Y,O,T).
                %
                %   - I: Id
                %   - X: X coordinate of position
                %   - Y: Y coordinate of position
                %   - O: Orientation
                %   - T: Earliest departure
                %
                %   agent_target(I,X,Y).
                %
                %   - I: Id of the agent
    ...
    def get_agent_position_literal(self, agent: Agent) -> str:
        ...
        """
        return f'agent_start({agent.id},{agent.position.x},{agent.position.y},{agent.orientation.value},{agent.earliest_departure}).'
